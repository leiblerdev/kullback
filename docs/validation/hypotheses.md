...
- **Status:** testing
- **Evidence:** Zauber founding engineer (Sebastian Lettner): "a lot of repetitive tasks" for customers — first interview-level confirmation that workload is heavy-tailed, ahead of the traffic-shape report. **DojoStack (Hung, 2026-09-03):** math Q-gen from pre-indexed ontology + pre-curated content — AI is a search engine generating on the fly from ontology. Extreme heavy-tail by design (curated ontology reused), but cost still ~zero because open-source + pooled subs.
...
- **Refinement (DojoStack, Hung Dang, 2026-09-03, call):** self-funded (sold prev co at 28, fuck-you money, can raise $60M) + 100k mostly-free users on $20-sub → **pre-monetization suppresses pain independently of credits/spend.** Open-source + "pool 10 subs" = inference ~free. Verdict: "bigger enterprise must be bigger pitch." Supports enterprise pivot in `criteria.md`; screen `paying users + P&L hit`, not just raw users/tokens.
...
- **DojoStack (Hung, 2026-09-03):** "don't use a router right now" + no benefit seen — graph-constrained generation (LangGraph, inject content, remove freedom) beats routing for accuracy. 4th independent no-router signal.
...
- **DojoStack (Hung, 2026-09-03):** evals = human army labelling math + pre-curated ontology. "Using human as judge at scale." Stack is LangGraph graph-engineering (inject content, constrain freedom) + own JSON UI with auto-fix artifacts. No automated per-task cheaper-model eval — humans ARE the eval. Confirms H12 (evals are the bottleneck, solved expensively with humans) + product TODO: GEPA in kullback ("cannot trust LLM to modify its prompt"), artifact auto-fix pattern.

### H15: AI-native startups are anti-buyers (candidate)
- **Claim:** AI-native pre-PMF startups ($20-sub, mostly free users, self-built AI stack) won't buy core infra: (1) no money / no P&L pressure, (2) won't let you touch core system, built everything themselves. Enterprise (paying volume, P&L hit) is the buyer.
- **Test:** "who owns inference infra, would you let an outside offline eval touch traces?" → "built it ourselves / no" kills.
- **Status:** candidate
- **Evidence:** DojoStack (Hung, 2026-09-03): "working with AI-native company is very hard — they don't have money, they won't allow me to touch core system, built everything with AI on their own. $20 subscription very hard to sell. Easier to sell service+product to enterprise — startup can die anytime." Matches Raban (Emdash): "more of an enterprise need" + Spherecast: "small companies focus on selling, not optimizing."
...
- 2026-09-03: DojoStack (Hung Dang, call) logged — self-funded, pre-PMF, 100k mostly-free users, zero inference pain (open-source + pooled subs ~free). Evals = human army + ontology + LangGraph graph-constraining. No router. Added H15 (AI-natives are anti-buyers, candidate); H2/H9/H12/H14 refined. Product TODO: GEPA + artifact auto-fix. Full notes in brain `gtm/docs/meeting-dojostack-hung.md`.
