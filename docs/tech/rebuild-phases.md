...
Landed 2026-09-02; the note is `phase-5-examiner.md`. The verifiers artifact and `task_status.json` over the fixture are byte-identical to phase 4's; `gates.json` gains the `trusted` row and `pipeline/state.json` loses the `derive_verifier` stage, both on purpose.
