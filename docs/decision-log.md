...
**D19. "We don't have gold."** Our gold is an approximation: the confirmed Reference's write calls executed on S0, atoms from re-roll agreement, with confidence (re-roll agreement, audit rate) published per Task. Lives in: eval-design.md step 7.
...
**D34. Founder's words are kept verbatim.** "Keep everything of my words please, those are the real decisions as well." `founder-words.md` holds every message unedited; this log indexes them.
...
Q35. Founder: "i would say read and then unclassified." Read as: the row defaults to `kind: read` so Runs stay gradeable, and it also carries `unclassified: true` (with `kind_confidence: low`) so the setup review (D48) sees it as an open row, not a settled one. Runs are not blocked. If the founder meant a two-step rule instead (read first, unclassified after some second signal), the log is wrong and should be corrected.
...
Q29, asked four times. Founder: "i would say this as well. we will need to discuss this more as well." Provisional: a write performed by the user during a Run is an End-state atom only when the customer's logs record it as an event. Then it carries `requestor: user` (tau3 banking shape), Provenance to its span (D42), and the Simulated user (D44) performs it at the recorded point. Writes the logs do not show do not exist for the Verifier, and the report says "user-side actions not in your logs, not graded". Grounds: D66 (raw trace is the source of truth), D42, D41.

Where research disagrees: tau3 banking grades user-side writes in every Task because its Simulated user has tools and the benchmark owns the world; a customer whose logs omit user actions gets a narrower Verifier from us. Trace-intake question 9 (outcome signals) is where we learn whether the logs carry them.
...
Q36. Founder: "i would say union of everything which is fixed eventually."
...
Q37. Founder: "i would say llm per column + code by rule ( which is verified by the LLM )".
...
Where research disagrees: tau2 has no column classes (whole-database hash); tau3 added canonicalization on top (R22). Column classes are our deviation; the cost is one review list per customer.

### D74. One world per customer, starting state per Task as an overlay (2026-08-27)

Q38 and Q39, settled over four turns. Founder first said "one s0 per task not per customer", then clarified: "we should be able to generate the world from the whole traces and then we come up with tasks in that world right ? and evaluate per task right ?" and accepted option 3 ("yes!").
...
- Emitting for tau2's harness (D56) merges overlays into one `db.json`; a conflict between two Tasks' overlays is a Gate failure for the tau2 export and the Task stays gradeable in our own Runner. Expected not to occur on tau2 retail.

Where research pulls the other way: tau2 and tau3 have one database per domain and tasks written to fit it; a shared world that is a patchwork of moments is our deviation, and a Candidate crossing Tasks can meet the same entity in two states. Rejected: latest-observation-only (loses the March Task), time-versioned rows (a clock in every generated tool body, and `valid_from` would be invented).
...
Q40. Founder: "i would say 1 + 2. but the rewriting should be check to be honest that is that the correct rewrite according to the traces we got."
...
Q42. Founder: "the answers should always be grouned in the real world to be honest ( which are the traces ) -> should always be consistent with the world".
...
Where research pulls the other way: tau2's user simulator is given a scenario text and instructed to invent nothing outside it, which is option 1 for everything not in the scenario; tau3's task-fix list (R22) includes user simulators that stonewalled agents on facts a real user would know. Reading from the world is our deviation and it is what closes that class of false failures.

### D78. The re-run count k is decided by experiment, not by design (2026-08-27)

Q43. Founder: "i mean we need to do experiments to do decide the number here right ?"
...
Q44. Founder: "i mean you should check it and then if there are something then the human checks right ? what does the research suggests ? what other are doign ?"
...
Q45. Founder: "we cannot fix that, we ideally need 100% 3. what do you suggest ? the world needs be to as close as to the real world sir." Also: "CUA-Gym forces an empty run to score below the gold run. The hacker-fixer paper ... i like these approaches."
...
- The target is 100%, not a percentage chosen to be reachable. Concretely:
  ...
  - Held-out calls, Verdict agreement, user fact consistency: also 100%, measured as "no unexplained miss". Every miss gets one reason recorded by code or the Builder: our bug (fixed), tau2's grader bug (tau3 shows 18.5% judge-human disagreement, R22), or ambiguous (goes to the setup review). A miss without a reason is a gate failure. The scorecard shows the raw number and the explained number side by side.
- The empty-run rule (CUA-Gym) and the hacker-fixer loop (2606.08960) are confirmed as parts of D79 (checks 3 and 6) and the founder's preferred approach to Verifier hardening.
- The wider intent, in the founder's words: the world must be as close to the real world as the traces allow; a number below 100% is a list of things to fix, not a score to accept.
...
Q46. Founder: "i would say we do it per task and then a proportion per task ?"
...
Q47. Founder: "several changes tested together what is the problem in that ? start with one change per round and then accept or reject and then several changes ?"
...
Q48. Founder: "two differnet tasks but the category could be same, tasks also have hierarchy right ?"
...
Where research pulls the other way: tau3 reads are never hashed and free-text fields are excluded from the DB hash altogether (R22), which is option 2; we grade them because a wrong recorded reason is a wrong End state (D43, the founder's cancel-and-ask-why example).

### D85. The report shows whether the Environment was built and the numbers; it suggests, the person decides (2026-08-27)

Q50. Founder: "report should also have that whether we were able to create the environment or not as well including the numbers and the verdict is decided by the the persons ( just show the numbers and suggest ) decision is the person."
...
- None of the three Q50 orderings was chosen as written; the founder's order is Environment status first, numbers second, suggestion last.

### D86. Spend ceiling hit: stop, report as is, ask before continuing (2026-08-27)

Q51. Founder: "we need to be as close as to the reality sir, report as it is is. stop and seek permission to continue further."
...
Q52. Founder: "lessons that it learnt from the previous customers sir ( so that it doesn't repeat ) but also ask it to question the relevance of the lessons as well."
...
Q54. Founder: "no! what do you recommend ?" Recommendation given after the answer: option 1, with the production-setting reading of option 3 folded in.
...
- If a customer's production system passes a label or context to their agent, it is in the traces as part of the prompt and the Candidate gets it the same way, because it is production input (D65), not Verifier output. Our Intent is never substituted for it.
...
Q55. Founder: "but our environment will envolve ( tau2 is the baseline right now, we will move further into more high fidelity better environment than tau2 ) so thinly wrap ? but lets first build the tau2 shape and then we build the openENV wrapper".
...
- The founder's framing recorded: tau2 is the baseline, not the destination; the Environment is expected to move past tau2 in fidelity, and the wrapper is what lets training frameworks consume it when it does.
...
Q56. Founder: "1. what do you suggest ?" Recommendation given after the answer: 1, with the one discipline from option 3.
...
- The re-runs count against the Builder's spend ceiling (D86) and each model call against the 40% cap (D65), which is where the founder wanted the cost seen.
...
Where research pulls the other way: R27's abstention section rests on human agreement targets (Trust or Escalate, SCOPE's conformal guarantee) that need a labeled set; without one, our disagreement rate has no proven error bound and must be reported as such. tau3 fixed 75 to 100 tasks with human review (R22); the founder's bet is that two agentic judges with tools plus the Verifier catch most of that class, and the tau2 slice is where the bet is measured (judge disagreement against tau2 reward).

### D93. A disputed Reference sets the Task aside until a person resolves it (2026-08-27)

Q58. Founder: "human resolves the dispute then sir."
...
- No third judge and no majority vote for References: a Reference sets the bar for every Verdict on its Task, and the founder wants that call made by a person when the machines split. Judge disagreement on other atoms (D76, D84, D88) still goes to the queue without stalling the Verdict, per D92.
...
Q59. Founder: "i mean we did the research right and that said that we should only care about the end state also we are not grading the process right now so."
...
Q60. Founder: "ask for the rest and reconstruct also i don't think any schema would be 9000 character long as well."
...
Q61. Founder: "1. i won't trust touching the rows as the selection 2. lets define what does coverage means? i would love to hear it in the form of total tasks covered ( which were defined early )."
...
Left open on purpose, because they need the founder: D71's three parts (Simulated user tools, user-side writes against sequence Hard constraints, required vs allowed for user-side writes). Everything else in the grill is closed; the next input is the vendor export and the tau2 slice.

## Pending (asked, not yet answered)

- D71 provisional (user-side writes); founder wants more discussion: Simulated user tools, interaction with sequence Hard constraints, required vs allowed. Three questions, to take when the founder wants them.
...
- First build done 2026-08-28 (756 tests, offline slice in `harness/SLICE_RESULTS.md`). Slice evidence that revises D97 defaults: cluster threshold 0.3 should be 0.6 (F1 0.276 versus 0.720); `propose_kind` needs a `generic` kind or a rule for it; error payloads need a per-source prefix rule (`Error: ` in tau2). Not applied yet; listed in `todo.md` under "From the first build and offline slice".
