...
tau2 for comparison: retail uses `NL_ASSERTION` in `reward_basis` on 112 of 114 tasks, airline uses `COMMUNICATE` on all 50, the third domain uses `ENV_ASSERTION` on all 2,285 (32 also `ACTION`). So an LLM judge is in the reward for tau2's largest conversational domain; I keep it out of the Verdict and confine it to the three places above.
