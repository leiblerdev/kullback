...
Things the design currently takes as true without proof. Each entry says what I assume, how strong the evidence is, what would show it wrong, and what breaks if it is. When an assumption is tested, I move it to the bottom section with the result. I add new assumptions the moment I make them, not later.

Evidence grades: **measured** (I or a cited source measured it on comparable data), **cited** (a published number from a different setting), **inferred** (reasoned from adjacent evidence), **none** (a guess I chose to proceed on).

## About the traffic we will see

| # | Assumption | Evidence | How I find out | What breaks |
|---|---|---|---|---|
...
| A3 | My first customers run support, email, calendar and ticketing agents. | none: my read of the market | First three signed customers. | Report 19's reference environments (tau2, AgentDojo, WorkArena) stop being the right templates; the generator's emit list stays valid. |
...
| A13 | Replaced by D48: setup review before any Verdict, blind audit after. Remaining assumption: the setup review is fast enough not to stall the first report. | none; decided 2026-08-27, still parked 2026-09-10. | Time the review per Task on the first customer. | If review takes days per Task, reports are human-paced and coverage must be cut to prominent Tasks. |
| A14 | The customer will actually read and correct Intents shown as Task names (decided D47; the behavior is still assumed). | none: confirmed as a decision on 2026-08-27, not yet observed, still parked 2026-09-10. | My decision; correction rate on the first customer. | If hidden, the customer cannot catch ungrounded Intents and the audit carries all of that load. |
...
| A16 | Substring communicate checks after normalization are reliable enough for numbers, names and ids. | cited, against me: "substring communication checks" is a named failure mode in 2607.02577. | Audit disagreements tagged to communicate atoms. | Move more communicate facts to the grounded-rubric judge (D25 place 2). |
...
| A20 | The frontier's recorded success is a meaningful bar. | cited, against me on hard Tasks: tau2 frontier pass^1 retail 74%, airline 56%, telecom 34%. | Absolute pass rates shown next to the delta. | "Non-inferior" can be true while both models are bad; the report must show the absolute number. |
...
- **A30** (2026-08-27, D78; re-checked 2026-09-10): the required atom set (D43) stabilizes within 10 frontier re-runs on most Tasks. Test: tau2 retail slice, n = 10 re-runs per Task, required set as a function of k. If false on more than 10% of Tasks, revisit the D43 definition before raising k. Status 2026-09-10: partly exercised. D112 landed re-rolls in the build (default 3 per Task), so the mechanism exists, but the 10-re-run stabilization test has not been reported; still open.

- **A31** (2026-08-27, D92; re-checked 2026-09-10): two agentic judges with tools plus the deterministic Verifier catch most Verifier and Reference defects without a human. Test: on the tau2 slice, judge disagreement rate and judge-vs-tau2-reward agreement per Task; on the first customer, judge verdicts against the customer's verdicts (D62). If agreement is below the D80 explained-100% bar, human review (D48) returns as required, not optional. Status 2026-09-10: contradicted as wired. Review R2 in docs/reviews/2026-09-03-codebase-review.md found both agentic judges are toolless in the CLI wiring and the third sample repeats the second, so the judges-with-tools setup this test needs does not hold there.
