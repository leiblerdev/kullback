...
- **GLM 5.3's environment-generation design, adopted as a target** (founder, 2026-08-29: "we also need to use this design for glm 5.3 for environment generation"). What was described: tasks that mirror expert work rather than puzzle problems, with the agent given the resources a person has (compute, storage, docs) and a long-horizon goal (find the bottleneck, run experiments, keep correctness); research agents convert task patterns from real workflows into runnable long-horizon environments; a judge agent checks each environment is solvable so no bad training signal enters; a verifier is generated per environment without access to the reference solution; solver trajectories close reward shortcuts and trivial solutions; every verifier passes three checks before use (the known-correct solution is rewarded, a run that did nothing is not, a run that left the task unfinished is not); high volume at high quality. From the release note itself (https://z.ai/blog/glm-5.3, added the same evening): "much of the difficulty in scaling post-training moves from the model to the environment"; a useful environment is executable, verifiable and close to real professional work, and many are needed, not a handful of hand-built ones; the environments carry multi-step dependencies and hidden state; the judge agent attempts each task itself to verify it is solvable; a verifier that passes the oracle, no-op and unsolved-state checks yields a binary reward reliable enough to train on directly; the pipelines still need a meaningful amount of human-in-the-loop work, and making generation and verification more autonomous is their next step (which is the loop item below). Their private Z.ai Code Bench scores agents on two dimensions, end-to-end completion and fine-grained checklist accuracy, and stays private against contamination: the Harness has the same two by construction (the Verdict and the per-atom result with the failing atom named, D46), and a customer's Environment is built from that customer's traces, so it is private the same way. Where the Harness already is: environments from real traces (the Builder), re-rolls of the frontier as solvability evidence (D112), the D79 suite with the oracle check (2) and the empty-Run check (3), the loophole probe as one solver trajectory against shortcuts (check 6). The founder, the same evening: "this judge agent which checks every environment is solvable is important, also we need a judge as an agent as well. there are 3 checks 1. oracle checks (must award reward) 2. null run check (agent did nothing, must award none) 3. incomplete check (must award none) and then we should have a trusted verifier as well." So three things are decided in principle and wait for the grill to shape them: a solvability judge that is an agent (it runs the Task, or reads the re-rolls, and says whether a frontier can finish it; a Task it refuses gets no Verifier and no training signal); the three checks as the entry condition of every Verifier, with "trusted Verifier" as the name of one that passed them (D79 checks 2 and 3 exist, the incomplete-Run check does not); and a judge agent in the loop, distinct from the D110 residue judge that only fails one side. What is missing, in order: ~~the incomplete-Run check in D79~~ (done as D119 the same evening: `verifier_unfinished_run`, the Reference cut before its last required write must score no pass); a solvability judge that reads the re-rolls per Task and refuses a Task no frontier Run finishes, rather than a gate that only counts finished Runs corpus-wide; many solver trajectories per Task feeding the probe, not one; long-horizon Tasks with resources beyond tool calls (files, docs, a compute budget), which the trace model does not carry yet; volume, which needs the loop item above and the DAG. One honest disagreement to settle in the grill: their verifier is written without the reference solution, ours is derived from it (D111), and the two guard against different failures (theirs against a verifier that only recognises one path, ours against a verifier grounded in nothing); an unseen-reference derivation with the Reference used only as the oracle check is the version that keeps both.
- **A deep grilling session to design the loop** (founder, 2026-08-29, last words of the evening: "we need to do a deep grilling session to design the loop and stuff"). Not a quick round: a full `/grill-with-docs` pass over the next item and the ones it pulls in (DAG execution, workers, the solvability judge as an agent, the trusted Verifier, verified synthetic data), one question at a time with a recommendation each, decisions into the log as they land. Questions already queued for it: the loop's shape (nested per step, or one flat round; recommendation nested), what each inner loop measures and what stops it, what the model may rewrite and what it may never touch (a gate, D110), how a rewritten prompt is versioned (D69), how the ratchet and the lesson are keyed, where the solvability judge sits and what it costs, and what "very very good" means as a number before the first round runs (principle 3).
- **The Builder as a loop that runs its own Environment and fixes what it finds** (founder, 2026-08-29, restated: "there has to be a loop which runs so that the harness can steer the creation of the environment"; grill started the same evening). D63 principle 4, D64 and design section 8 already decide this in principle (the model proposes edits to its prompts, checklists, gates and seed corpus, an evaluator outside the loop accepts them, Builder history is a tree); no code runs it. Today the pipeline is one pass with bounded repair inside single stages, and `--iterate` only resumes the cache. The founder wants a loop over the whole thing: 1. build the Environment, 2. augment it (grow the world, add the rows and error paths the traces only hint at), 3. generate the Tasks, 4. harden the Tasks (probe them, find the loophole, the unreachable Reference, the Verifier an empty Run passes, and fix the cause), then round again until the Environment is very good. The model inside the loop should be able to change its own system prompts, checklists and seed corpus (principle 8 in the design), run the Environment, read where a Run went wrong and repair the artifact, with every change gated by code as now. The first two mechanisms, from retail build 7 (2026-08-29; founder: "you know for this reason we need loops"): build 5 had a `calculate` body that passed every gate, build 7 re-ran compile_tools and the model drew a worse sample (eval refused by confinement, then a parser that broke on parentheses, then `decimal` without its import), so the tool ended assisted and four re-rolled writes differed. (1) A ratchet: a body that cleared the gates in an earlier build of the same workdir is kept until a new body clears them too; a stage never replaces a passing artifact with a failing one. (2) A lesson: the sequence of gate failures a tool went through is written to the Builder memory (`judge_lessons`, `memory.py`) and read into the next build's first attempt for that tool, so the model does not repeat the same three mistakes. Both are code, both are cheap, both belong to step 1 of the loop. Open questions for the grill: what "good" is measured by per round (D79 suite pass rate, re-roll agreement, assisted count, tau2 reward agreement while the scaffold exists), what stops the loop (a round that improves nothing, a spend ceiling), what the model may and may not rewrite (never a gate, D110), and how a rewritten prompt is versioned so a regrade can name it (D69 content hashes).
