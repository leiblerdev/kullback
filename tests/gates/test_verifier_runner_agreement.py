...
Verdict time, so this file pins them to the same answer on the Runs the gates are made of. The
Verifiers are derived by the Builder and scored by the gates package and the Runner; the module
under test imports neither the Builder nor the Runner's grader (D89, D91, D122).
"""
...
@pytest.mark.parametrize("run_factory, expected_pass", [
    pytest.param(reference_run, True, id="oracle"),
    pytest.param(alt_path_run, True, id="second_path"),
    pytest.param(empty_run, False, id="empty"),
    pytest.param(wrong_run, False, id="plausible_wrong"),
])
def test_the_runs_the_d79_checks_are_made_of_score_the_same_under_both_scorers(
        tmp_path, run_factory, expected_pass):
    agree(derive(tmp_path), run_factory(), expected_pass)
...
def test_a_question_the_reference_asked_in_its_own_words(tmp_path):
    ...
    assert S.atom_payload(question)["key"] == "field:reason"
