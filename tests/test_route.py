...
from harness.runner.route import (
    Router,
    StateView,
    canonical_args,
    recording,
...
from harness.shared.records import ErrorShape, OverlayRow, TaskOverlay, ToolSig


def fake_tools() -> types.ModuleType:
...
def make_router(**kwargs) -> Router:
    ...
    kwargs.setdefault("env_tools_module", fake_tools())
...
def test_an_overlay_given_beside_a_state_view_is_not_dropped():
    ...
    overlay, rows = pinned_overlay()
    router = Router(env_tools_module=fake_tools(), starting_state=StateView(shared=shared()),
                    overlay=overlay, overlay_rows=rows, tool_sigs=sigs())
    assert router.route("get_order_details", {"order_id": "123"}).result["status"] == "pending"
...
def test_a_missing_entity_is_not_found_entity():
