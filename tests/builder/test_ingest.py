...
def test_a_derived_trace_carries_its_ids_source_ingest_version_turns_and_calls(small_file, workdir):
    raw = ingest.store_raw(small_file, workdir)
...
def test_each_error_message_is_classed_by_rule_and_keeps_its_payload_verbatim(text, expected):
...
def test_a_result_that_ends_in_a_truncation_marker_is_marked_with_the_marker_it_ends_in(text, marker):
...
def test_the_rule_the_message_reads_like_wins_over_the_rule_a_word_of_it_would_fire(text, expected):
