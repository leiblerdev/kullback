...
from kullback.runner.records import EntitySchema, RawPtr, ToolCall, Trace, Turn
...
def test_the_prose_results_of_another_requestor_are_grouped_by_tool_and_the_assistants_are_not():
...
def test_a_reader_that_fails_is_named_once_per_tool_and_masked_shape(tmp_path):
    broken = """
def read(result):
...
"""
    ...
    body = json.dumps({"table": "greenhouse", "columns": [{"name": "vent"}],
                       ...
                       "renders": [{"tool": "check_vent", "source": VENT_RENDER}]})
    _, ruling = gated(tmp_path / "per-shape", body, traces=traces)
...
def _always_opened():
    ...
    return [trace(f"run_{i}", [call("check_vent", "Vent: CLOSED"),
                               call("open_vent", "Vent opened."),
                               call("check_vent", "Vent: OPEN")]) for i in range(4)]


def _opened_once():
    """One changing interval is not a corpus saying anything."""
    return ([trace("open_0", [call("check_vent", "Vent: CLOSED"),
                              call("open_vent", "Vent opened."),
                              call("check_vent", "Vent: OPEN")])]
...
@pytest.mark.parametrize(("traces", "credited"), [
    (credit_traces, {"open_vent": ["vent"]}),
    (_always_opened, {}),
    (_opened_once, {}),
], ids=["mostly-moves-around", "never-run-without", "seen-too-rarely"])
def test_only_a_tool_the_column_mostly_moves_around_is_credited_with_changing_it(tmp_path, traces, credited):
    """A check that restates the whole state sits inside changing and unchanging intervals alike and
    is credited with nothing; the warmth moves whatever was called, so no call explains it."""
    proposal, _ruling = gated(tmp_path, proposal_json(), traces=traces())
    assert proposal.effects == credited
...
def test_the_starting_row_is_what_was_read_before_the_write_and_not_after_it(tmp_path):
    ...
    # with no write seen to change it, the first reading of a column stands
    proposal, ruling = gated(tmp_path / "no-write", proposal_json(), effects={})
    rows = readers.starting_rows(two_traces(), proposal, ruling.parsed)
...
def test_a_column_a_recording_never_read_before_writing_is_filled_from_the_corpus(tmp_path):
    ...
    # the fill is the value the most recordings showed before their first write
    traces = three_traces() + [trace("run_d", [call("check_vent", "Vent: OPEN")])]
    proposal, ruling = gated(tmp_path / "majority", proposal_json(), traces=traces, effects={"open_vent": ["vent"]})
    rows = readers.starting_rows(traces, proposal, ruling.parsed)
...
def test_after_the_last_attempt_the_proposal_with_the_fewest_failing_shapes_is_kept_and_assisted(tmp_path):
    ...
    # a render that cannot be satisfied leaves its proposal kept and assisted too
