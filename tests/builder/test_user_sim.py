...
def test_the_rules_are_incomplete_only_when_an_asked_field_went_unanswered_or_no_user_spoke():
    rules = derive_user_rules(
        make_trace(
            [
                ("assistant", "What is your zip code?"),
...
def test_derive_over_the_fixture_finds_the_recorded_zip_and_name_and_complete_rules(tau2_small):
...
def test_a_turn_asks_for_exactly_the_fields_its_wording_requests(tau2_small):
    ...
    # However the agent words the question, a fact the recording holds is given.
...
def test_wording_that_drops_or_adds_a_fact_is_replaced_by_the_plain_sentence(rules_with_zip, make_test_model):
...
def test_an_on_request_fact_is_given_once_asked_and_not_before(rules_with_zip):
    user = SimulatedUser(rules_with_zip)
...
def test_the_world_is_read_as_this_or_the_named_user_and_not_as_the_first_row(tau2_retail_dir):
...
def test_the_model_is_not_asked_to_word_an_empty_reply_or_an_unavailable_fact(make_test_model):
...
def test_mined_facts_are_only_values_the_user_gave_to_a_call_that_succeeded(tau2_small):
...
def test_the_user_restates_its_goal_once_when_silent_or_asked_if_that_is_all_then_closes():
...
def test_a_candidate_that_closes_over_or_passes_on_an_unfinished_goal_ends_handed_off():
    user = renewal_user(write_tools={"renew_loan"}, goal_writes={"renew_loan"})
    user.reply(ask("Hi! How can I help you today?"))
...
def test_a_turn_that_answers_one_field_and_misses_another_is_not_an_end():
    ...
    user = renewal_user(write_tools={"renew_loan"}, goal_writes={"renew_loan"})
    user.reply(ask("Can you confirm the loan number and your member id?"))
    assert said(user)["member_id"] == "unavailable"
    assert user.done is False
    assert user.end_reason is None
...
def test_a_run_that_spends_its_turns_without_ending_gave_up(make_test_model):
    ...
    user = renewal_user(write_tools={"renew_loan"}, goal_writes={"renew_loan"})
    model = make_test_model([{"content": "Which loan would you like to renew?"}], loop=True)
    state = loop.new_run_state("spent", user=user, max_turns=2)
    loop.run(state, model)
...
def test_a_write_happened_only_when_its_column_moved_or_its_replay_effect_check_passed():
