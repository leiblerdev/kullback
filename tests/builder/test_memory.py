...
from kullback.builder.memory import (
    AnonymizationError,
    Lesson,
    LessonApplication,
    LessonError,
...
from kullback.runner.records import FieldStat, SetAsideLesson, ToolSig, content_hash
...
def test_loading_a_node_that_is_not_on_disk_raises_node_not_found_rather_than_returning_an_empty_one(workdir):
...
def test_judge_relevance_asks_the_model_once_with_the_condition_and_the_evidence(make_test_model):
    model = make_test_model(['{"relevant": true, "evidence": "search_products returns a list"}'])
...
def test_judge_relevance_reads_a_no(make_test_model):
    model = make_test_model(['{"relevant": false, "evidence": "no tool returns a list"}'])
    relevant, evidence = judge_relevance(model, _lesson(), toolsigs=[], policy_spans=[])
    assert relevant is False
    assert evidence == "no tool returns a list"


def test_judge_relevance_needs_evidence_before_it_says_yes(make_test_model):
    model = make_test_model(['{"relevant": true, "evidence": ""}'])
    relevant, evidence = judge_relevance(model, _lesson(), toolsigs=[], policy_spans=[])
    assert relevant is False
    assert "evidence" in evidence


def test_judge_relevance_sets_aside_an_unparseable_reply(make_test_model):
    model = make_test_model(["I am not sure what you mean"])
    relevant, evidence = judge_relevance(model, _lesson(), toolsigs=[], policy_spans=[])
    assert relevant is False
    assert "I am not sure" in evidence


def test_judge_relevance_accepts_a_quoted_policy_span_and_refuses_a_pointer_to_one(make_test_model):
    spans = [{"text": "refund only within 30 days"}]
    model = make_test_model(['{"relevant": true, "evidence": "refund only within 30 days"}'])
    ...
    pointer = make_test_model(['{"relevant": true, "evidence": "policy line 3"}'])
    relevant, reason = judge_relevance(pointer, _lesson(), toolsigs=[], policy_spans=spans)
    assert relevant is False
    assert "quotes nothing" in reason


def test_judge_relevance_never_builds_its_own_model(make_test_model):
    ...
    model = make_test_model(['{"relevant": true, "evidence": "get_user"}'])
...
def test_proposal_that_touches_a_read_only_file_is_refused(workdir):
    ...
    with pytest.raises(ReadOnlyEditError):
        propose(workdir, "widen the schema", "p", edit_paths=["kullback/runner/loop.py"])
    with pytest.raises(ReadOnlyEditError):
...
def test_the_read_only_list_is_the_one_the_design_names(workdir):
    assert set(memory.READ_ONLY_PATHS) == {
        ...
        "runner/budget.py", "runs/", "data/raw/",
    }
    assert memory.read_only_hits("edit builder/mine.py") == []
    assert memory.read_only_hits(["kullback/runner/route.py"]) == ["kullback/runner", "route.py"]
    # D122: the gates package as a whole, not only the Runner's files, is off limits to the agent.
    assert memory.read_only_hits(["kullback/gates/artifacts.py"]) == ["gates/", "kullback/gates"]
    assert memory.read_only_hits("the build's gates.json") == []


def test_an_edit_under_the_builder_directory_is_allowed(workdir):
    node = propose(workdir, "widen the result schema in mine.py", "p",
                   edit_paths=["kullback/builder/mine.py"], edit_kind="mine")
    assert node.edit_paths == ["kullback/builder/mine.py"]
...
def test_a_retired_lesson_is_never_judged_relevant_and_costs_no_model_call(make_test_model):
    model = make_test_model(['{"relevant": true, "evidence": "get_user"}'])
...
def test_a_lesson_with_no_relevance_condition_is_set_aside_without_a_model_call(make_test_model):
    model = make_test_model(['{"relevant": true, "evidence": "get_user"}'])
...
def test_evidence_that_quotes_nothing_from_the_material_is_refused(make_test_model):
    model = make_test_model(['{"relevant": true, "evidence": "trust me, it applies"}'])
...
def test_evidence_may_quote_a_result_field_name(make_test_model):
    sigs = [ToolSig(name="get_user", result_schema=[FieldStat(name="membership_tier")])]
    model = make_test_model(['{"relevant": true, "evidence": "membership_tier is a list"}'])
    relevant, _ = judge_relevance(model, _lesson(), toolsigs=sigs, policy_spans=[])
    assert relevant is True


def test_only_a_json_true_counts_as_relevant(make_test_model):
    for answer in ['"false"', '"true"', '1', 'null', '"yes"']:
        model = make_test_model(['{"relevant": %s, "evidence": "get_user"}' % answer])
...
def test_a_set_aside_lesson_always_carries_a_reason(make_test_model):
    model = make_test_model(['{"relevant": false}'])
    relevant, reason = judge_relevance(model, _lesson(), toolsigs=[], policy_spans=[])
    assert relevant is False
    assert reason.strip()


def test_judge_lessons_splits_applied_from_set_aside_for_the_report(workdir, make_test_model):
    keep = _lesson()
