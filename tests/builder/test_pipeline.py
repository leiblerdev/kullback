...
from kullback.runner.records import Category, GateResult, Task
...
def test_a_second_run_of_an_unchanged_stage_is_served_from_the_cache_without_calling_the_stage_again(workdir):
    fn = make_counter()
...
def test_a_record_a_stage_returned_comes_back_from_the_cache_as_that_record_and_not_as_its_repr(workdir):
...
def test_a_gate_that_passes_is_recorded_on_the_result_and_the_pipeline_completes(workdir):
    result = Pipeline(
...
def test_the_anchor_holds_back_a_share_of_each_tasks_runs_with_a_floor_and_marks_a_task_too_small_to_hold_any_unguarded(workdir):
    tasks = [
...
def test_the_charge_that_reaches_the_ceiling_stops_the_build_where_it_stands_and_reports_the_item(workdir):
    """The shipped budget.Ceiling, not a copy of its rules: the third 0.4 charge reaches 1.0 USD."""
    reached = []
...
def test_a_ceiling_that_is_already_reached_stops_the_build_before_the_first_stage_runs(workdir):
    ran = []
    ...
    # A ceiling is exhausted by being charged, never by assigning to it; one built already reached
    # is refused outright, so a resumed build cannot buy one more item before stopping again.
    with pytest.raises(budget.BudgetExceeded):
        budget.Ceiling(usd=1.0, spent=1.0)
    ceiling = budget.Ceiling(usd=1.0)
    with pytest.raises(budget.BudgetExceeded):
        ceiling.add(1.0, "an_earlier_stage", "tool_0")
...
def test_charge_without_a_ceiling_is_a_no_op(workdir):
    ...
    result = Pipeline([Stage("s", spender, [], ["a"])], workdir=workdir).run()
    assert result.status == "complete" and result.stopped is None
    assert result.artifacts["a"] == 1, "the stage ran to its end and released what it made"
    assert not (Path(workdir) / "budget.json").exists(), "no ceiling, so nothing recorded a spend"


# --- mermaid ----------------------------------------------------------------

def test_the_mermaid_diagram_shows_every_stage_pending_before_a_run_and_its_outcome_after(workdir):
    stages = [
...
def test_mermaid_marks_a_stopped_stage(workdir):
    ...
    pipe = Pipeline([Stage("s", spender, [], ["a"])], workdir=workdir, ceiling=budget.Ceiling(usd=1.0))
    pipe.run()
...
def test_pipeline_does_not_import_the_builder():
    ...
    pipeline.py moved into builder/ with the rest of the Builder's own code in phase 1 (D130); it is
    no longer part of the runner/ and ai/ surface this gate scans, so the point now is that the gate
    still passes on the real repo and never has reason to name it.
    """
    from kullback.runner import validate

    root = Path(__file__).resolve().parents[2]
    gate = validate.import_boundary_check(root)
    assert gate.passed, gate.failures
    assert not any("pipeline.py" in failure for failure in gate.failures)


def test_the_boundary_gate_would_catch_a_dynamic_builder_import_from_the_runner(tmp_path: Path):
    """The guard above is only worth something if the gate catches what it is guarding against.

    pipeline.py moved out of the scanned runner/ surface in phase 1 (D130), so this plants the same
    dynamic-import escape in a real runner/ file (loop.py) instead.
    """
    from kullback.runner import validate

    root = tmp_path / "kullback"
    (root / "runner").mkdir(parents=True)
    source = (Path(__file__).resolve().parents[2] / "kullback" / "runner" / "loop.py").read_text(
        encoding="utf-8")
    extra = "\n\ndef sneak():\n    import importlib\n    return importlib.import_module('kullback' + '.builder')\n"
    (root / "runner" / "loop.py").write_text(source + extra, encoding="utf-8")
...
def _events(pipeline, artifacts=None):
    """The events one run emitted. A run that raises is the caller's to expect, never swallowed here."""
    seen = []
...
def test_a_crash_names_the_stage_it_died_in_before_the_exception_leaves(tmp_path):
    ...
    def boom(ctx, inputs):
        raise ValueError("no")
    pipe = Pipeline([Stage("first", boom, outputs=["a"])], tmp_path, on_event=seen.append)
