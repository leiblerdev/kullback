...
def test_booleans_and_none_canonicalize_to_their_json_words_and_true_is_not_one():
...
def test_whitespace_collapses_and_case_folds_the_tau3_way():
...
def test_default_rules_are_not_shared_state():
    ...
    from kullback.runner.canon import DEFAULT_RULES
...
def test_a_comparison_with_no_cache_key_records_no_use(tmp_path):
    ...
    from kullback.runner.canon import USES_FILE, record_use

    record_use(tmp_path, compare("a", "a", "hard"), run_id="r1")
    assert not (tmp_path / USES_FILE).exists()

    table = EquivalenceTable()
    put(table, "tickets.reason", "a b", "c d", True, "llm", "1")
    cached = compare("a b", "c d", "semantic", table=table, column="tickets.reason")
    record_use(tmp_path, cached, run_id="r1")
    rows = [json.loads(line) for line in
            (tmp_path / USES_FILE).read_text(encoding="utf-8").splitlines() if line.strip()]
...
def test_a_judge_result_verdict_decides_the_comparison():
    ...
    said_no = compare(
        ...
        judge=lambda c, a, b: JudgeResult(use="equivalence", verdict="not_equivalent", reason="different complaints"),
    ...
    said_yes = compare(
        "a b", "c d", "semantic",
        judge=lambda c, x, y: JudgeResult(use="equivalence", verdict="equivalent"), table=table, column="c",
    )
...
def test_an_abstaining_judge_leaves_the_pair_unresolved_and_uncached():
    ...
    result = compare(
        "a b", "c d", "semantic",
        judge=lambda c, x, y: JudgeResult(use="equivalence", verdict="abstain"), table=table, column="c",
    )
    assert result.equal is False
    assert result.route == "unresolved"
    assert result.judge_used is False
...
def test_a_cached_human_decision_is_not_judge_used():
    table = EquivalenceTable()
