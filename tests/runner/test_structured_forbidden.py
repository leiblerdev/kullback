...
def test_structured_forbidden_write_agrees_between_gate_and_verdict(matches):
    ...
    assert target.check_run(verifier, run, CanonRules(), write_tools={CANCEL}) == (not matches, "ban" if matches else None)
...
def test_whole_tool_prohibition_covers_calls_with_entity_ids(entity, matches):
    ...
    assert target.atom_holds(atom, run, target.canon_fn(CanonRules()), {CANCEL}) is matches
    assert target.check_run(verifier, run, CanonRules(), write_tools={CANCEL}) == (not matches, "ban" if matches else None)
    result = verdict(run, verifier, write_tools={CANCEL})
    assert result.passed is not matches
    assert result.class_ == ("fail" if matches else "pass")
