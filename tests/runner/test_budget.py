...
@pytest.mark.parametrize(
    "tokens, refused", [(39_000, False), (40_000, False), (40_001, True)]
)
def test_the_context_cap_refuses_only_a_call_past_forty_percent(tokens, refused):
    if not refused:
        assert budget.check_context_cap(tokens, window=100_000) is None
        return
    with pytest.raises(budget.ContextCapExceeded) as excinfo:
        budget.check_context_cap(tokens, window=100_000)
...
def test_the_context_cap_is_forty_percent_of_the_window():
...
def test_budget_does_not_import_the_builder():
    """A grep for the word misses an aliased, built or exec'd import, so walk the import graph."""
    from test_verdict import import_closure

    closure = import_closure("kullback.runner.budget")
    assert "kullback.runner.budget" in closure
