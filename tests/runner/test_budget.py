...
@pytest.mark.parametrize(
    "usage, expected",
    [
        (Usage(input=1_000_000), lambda price: price["input"]),
        (
            Usage(input=1_000_000, cache_read=400_000, cache_write=100_000),
            lambda price: price["input"] + 0.4 * price["cache_read"] + 0.1 * price["cache_write"],
        ),
        (
            Usage(input=10, cache_read=7, cache_write=2),
            lambda price: (10 * price["input"] + 7 * price["cache_read"] + 2 * price["cache_write"]) / 1e6,
        ),
    ],
    ids=["a_million_input_tokens", "input_beside_cache_reads_and_writes", "mostly_cached"],
)
def test_each_token_count_is_billed_once_at_its_own_rate(usage, expected):
    ...
    assert budget.call_cost(usage, "anthropic/claude-opus-5") == pytest.approx(expected(price))
...
def test_record_call_fills_the_cost_and_writes_per_stage_and_per_build_totals(workdir):
    first = budget.record_call(call_event(input=1_000_000), stage="mine", workdir=workdir)
    assert first.cost.usd == pytest.approx(budget.PRICES["anthropic/claude-opus-5"]["input"])
    ...
    assert totals["stages"]["mine"]["wall_ms"] == pytest.approx(240.0)
    ...
    budget.record_call(call_event(model="openai/mystery", input=100), stage="unpriced", workdir=workdir)
    totals = budget.load_totals(workdir)
    assert totals["stages"]["unpriced"]["unpriced_calls"] == 1
    assert totals["stages"]["unpriced"]["wall_ms"] == pytest.approx(120.0)


def test_the_cache_effect_is_what_reads_saved_over_the_input_rate_less_the_write_premium(workdir):
    price = budget.PRICES["anthropic/claude-opus-5"]
    ...
    # The ledger carries what the cache saved beside what was paid.
    ...
    assert bucket["cache_saved_usd"] == pytest.approx(reads)
...
@pytest.mark.parametrize(
    "tokens, window, fraction, refused",
    [
        (39_000, 100_000, None, False),
        (40_000, 100_000, None, False),
        (40_001, 100_000, None, True),
        (500, 1_000, 0.5, False),
        (501, 1_000, 0.5, True),
    ],
    ids=["under", "at_the_cap", "one_past", "configured_at_the_cap", "configured_one_past"],
)
def test_the_context_cap_refuses_only_a_call_past_forty_percent(tokens, window, fraction, refused):
    """Forty percent of the window by default; a fraction passed in moves the cap."""
    assert budget.context_cap_tokens(200_000) == 80_000
    kwargs = {} if fraction is None else {"fraction": fraction}
    cap = int(window * (fraction or 0.4))
    if not refused:
        assert budget.check_context_cap(tokens, window=window, **kwargs) is None
        return
    with pytest.raises(budget.ContextCapExceeded) as excinfo:
        budget.check_context_cap(tokens, window=window, **kwargs)
    message = str(excinfo.value)
    assert str(tokens) in message and str(cap) in message


# --- the spend ceiling (D86) ---


def test_ceiling_lets_spending_through_under_the_limit_and_raises_past_it_with_stage_item_spent_and_estimate():
    under = budget.Ceiling(usd=1.0)
    assert under.add(0.25, stage="compile_tools", item="get_order") == pytest.approx(0.75)
    assert under.spent == pytest.approx(0.25)
    assert under.remaining == pytest.approx(0.75)
    assert under.add(0.25, stage="compile_tools", item="get_user") == pytest.approx(0.5)
    assert under.stage_spend["compile_tools"] == pytest.approx(0.5)
    assert under.stage_charges["compile_tools"] == 2
...
def test_a_ceilinged_build_refuses_a_model_it_cannot_price(workdir, make_test_model):
    ...
    # The wrapper refuses before the first call.
...
def test_a_ceiling_resumes_from_the_totals_file_unless_that_file_already_reaches_it(workdir):
    ...
    # The ceiling's own spend lands in the same file a resume reads.
    fresh = workdir / "direct"
    budget.Ceiling(usd=1.0, workdir=fresh).add(0.9, stage="mine", item="a")
    assert budget.Ceiling.from_totals(fresh, usd=2.0).spent == pytest.approx(0.9)
...
def test_record_call_charges_the_ceiling_from_the_ledger_without_counting_twice(workdir):
    ...
    breached = workdir / "breached"
    ceiling = budget.Ceiling(usd=12.0, workdir=breached)
    for idx in range(2):
        budget.record_call(call_event(idx=idx, input=1_000_000), stage="mine", workdir=breached, ceiling=ceiling)
    with pytest.raises(budget.BudgetExceeded):
        budget.record_call(call_event(idx=2, input=1_000_000), stage="mine", workdir=breached, ceiling=ceiling)
    assert ceiling.spent == pytest.approx(15.0)
    with pytest.raises(budget.BudgetExceeded):
        budget.record_call(call_event(idx=3, input=1_000_000), stage="mine", workdir=breached, ceiling=ceiling)
    assert ceiling.spent == pytest.approx(20.0), "spent must reflect the ledger even when charge_recorded refuses"
    assert budget.load_totals(breached)["total"]["usd"] == pytest.approx(20.0)
...
def test_every_call_through_the_wrapper_is_priced_and_written_to_the_ledger(workdir, make_test_model):
    from kullback.ai.provider import ModelReply
...
def test_a_memomodel_hit_is_counted_in_the_memo_hits_bucket_and_priced_at_zero(workdir, make_test_model):
    """docs/prompt-caching.md item 3: a hit still counts as a call, but adds to memo_hits and
    prices at zero because MemoModel already zeroed its usage. A plain model never adds to it."""
    from kullback.ai.provider import MemoModel, ModelReply, TestModel
    ...
    plain = budget.BudgetedModel(make_test_model(["ok"], loop=True), stage="plain", workdir=workdir,
                                 model_id="anthropic/claude-opus-5")
    plain.query([{"role": "user", "content": "hi"}])
    assert budget.load_totals(workdir)["stages"]["plain"]["memo_hits"] == 0


@pytest.mark.parametrize("caller_key, expected", [(None, "kullback-abc-mine"), ("caller-set", "caller-set")],
                         ids=["unset_takes_the_wrapper_key", "set_is_left_alone"])
def test_the_wrapper_sets_the_prompt_cache_key_only_when_the_caller_left_it_unset(
        workdir, make_test_model, caller_key, expected):
    ...
    config = None if caller_key is None else ModelConfig(prompt_cache_key=caller_key)
    model.query([{"role": "user", "content": "hi"}], config=config)
    assert inner.calls[0]["config"].prompt_cache_key == expected
...
def test_a_call_is_priced_under_its_provider_not_the_wire_id_the_endpoint_echoed(workdir):
    ...
    event = budget.record_call(Event(idx=0, type="model_call", cost=cost), stage="mine", workdir=workdir)
    assert event.cost.usd == pytest.approx(budget.call_cost(cost.usage, "openai/gpt-5.6-luna"))
    rows = [json.loads(line) for line in feed.path_for(workdir).read_text(encoding="utf-8").splitlines()]
    assert [row["model"] for row in rows if row["kind"] == "model_call"] == ["openai/gpt-5.6-luna"]
    # An id that already names its provider, or a Cost with no provider, is left alone.
...
def test_the_default_model_is_priced_and_has_its_own_window_offline():
    from kullback.ai.provider import DEFAULT_MODEL

    assert budget.price_for(DEFAULT_MODEL) is not None
    assert budget.window_for(DEFAULT_MODEL) != budget.DEFAULT_CONTEXT_WINDOW


# --- the harness subscriber ---


def _assistant_end(usage, model=None):
    from kullback.agent.events import MessageEndEvent
    from kullback.ai.messages import AssistantMessage

    return MessageEndEvent(message=AssistantMessage(content="ok", usage=usage, model=model))


def test_the_subscriber_prices_one_assistant_message_into_the_ledger_under_its_stage(workdir):
    model_id = next(iter(budget.PRICES))
    price = budget.subscriber(workdir, "builder_session", model_id)
    price(_assistant_end(Usage(input=1890, output=17, cache_write=1887)))
    totals = budget.load_totals(workdir)
    bucket = totals["stages"]["builder_session"]
    assert (bucket["calls"], bucket["input"], bucket["output"], bucket["cache_write"]) == (1, 1890, 17, 1887)
    assert bucket["usd"] > 0
    assert totals["total"]["usd"] == bucket["usd"]


def test_the_subscriber_ignores_a_message_without_usage_and_the_turn_end_that_repeats_one(workdir):
    from kullback.agent.events import TurnEndEvent
    from kullback.ai.messages import AssistantMessage

    price = budget.subscriber(workdir, "builder_session", next(iter(budget.PRICES)))
