...
def test_overlay_and_user_behaviour_stub_exist():
    ...
    behaviour = UserBehaviour(behaviour_id="b1", style_notes=["short sentences"])
    assert UserBehaviour.model_validate(as_dict(behaviour)) == behaviour
...
def test_content_hash_of_a_set_does_not_move_between_processes():
...
def test_content_hash_of_a_set_is_the_same_in_a_fresh_interpreter():
    ...
    code = "from harness.shared.records import content_hash; print(content_hash({'b','a','c','d','e'}))"
    digests = set()
    for seed in ("1", "2", "3"):
