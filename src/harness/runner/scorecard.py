...
from harness.runner.gate_support import MISS_REASONS, _get, _passed, _rate, _same, _share, gate
from harness.shared.records import as_dict, content_hash
...
def scorecard(build_dir: Union[str, Path], reference_verdicts=None) -> dict:
    ...
    # Deferred: validate.py imports scorecard() from here to re-export it, so a module-level import of
    # validate.py back into this file would be circular. By call time both modules are fully loaded.
    from harness.runner.validate import replay_fidelity_gate
