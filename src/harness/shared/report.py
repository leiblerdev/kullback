...
# SetAsideLesson is imported from records.py above and re-exported here, because every record lives
# in records.py (build brief rule 4) and memory.py builds this list for the report.
...
def _pending_review(data: ReportData) -> list[str]:
    ...
    repeated over the records because shared/ does not import builder/ (D89).
    """
...
def _uncounted_line(data: ReportData, record: Verdict) -> str:
    """One Verdict left out of the numbers, with the mark or the tool that excluded it (D49, D88)."""
    run = next((r for r in data.runs if r.run_id == record.run_id), None)
    reasons = []
...
def _task_numbers_lines(data: ReportData, numbers: dict) -> list[str]:
    ...
    if uncounted:
        ...
        lines += [_uncounted_line(data, record) for record in uncounted]
    return lines
...
def scorecard_rows(body: Any) -> list[ScorecardItem]:
    """The scorecard as rows, from a list of ScorecardItem or from validate.scorecard's own dict (D62, D80).

    validate.py returns one nested dict per build, which is the only scorecard any module produces,
