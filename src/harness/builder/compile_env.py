...
def gate_replay_fidelity(sandbox: Sandbox, calls: Iterable[ToolCall], schema: EntitySchema,
    ...
    for call, result in zip(calls, results, strict=False):
        ...
        if not result["ok"]:
            ...
            continue
        ok, differing = _compare(schema, _parse_result(call.result), result["value"], rules)
...
def compile_tool(model, toolsig: ToolSig, calls: Iterable[ToolCall], schema: EntitySchema, db: dict,
                 ...
                 call_states: Optional[dict] = None, rules: Any = None) -> ToolBuild:
    ...
    defaults and can fail a body over a difference the customer's own rules fold away.
