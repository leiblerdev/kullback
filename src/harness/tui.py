...
class Board:
    ...
    def verdict(self) -> Text:
        ...
        if failed:
            ...
            first = failed[-1]
            reason = "; ".join(first.get("failures") or []) or "no reason given"
            out.append(f"\n  {first.get('stage')}: {reason[:160]}", style="red")
        if self.outcome:
...
class Screen:
    ...
    def _adapter(self) -> Any:
        ...
        if self.model is None:
            return None
        from harness.shared import provider
        provider.load_dotenv()
        if not provider.enable_live_calls_from_env():
            raise RuntimeError("live model requests are off; set HARNESS_ALLOW_MODEL_REQUESTS=1 in .env")
        return provider.model_for(self.model, self.base_url)
    ...
    def _status(self) -> None:
        ...
        board.gates = [{"stage": g.get("stage"), "passed": g.get("passed"),
