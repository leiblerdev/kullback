...
from kullback.runner.world.recorded import TIME_PATTERN, context_feed_key, is_id_key, result_leaves
from kullback.runner.world.recorded import holdout_values as holdout_values
from kullback.runner.world.recorded import recorded_call_context as recorded_call_context
from kullback.runner.world.recorded import recorded_call_contexts as recorded_call_contexts
...
# The generated module stays self-contained (it cannot import the harness), so the shim
# carries the runner's TIME_PATTERN as text: one placeholder, replaced once, at module level.
_CONTEXT_SHIM = _CONTEXT_SHIM.replace("__TIME_PATTERN__", TIME_PATTERN, 1)


def _context_entity_ids(node: Any) -> set:
    return {value for key, value in result_leaves(node)
            if isinstance(value, str) and is_id_key(key)}
...
def recorded_run_context(calls: Iterable[ToolCall], schema: EntitySchema,
    ...
    for call in calls:
        ...
        arg_strings = {value for _, value in result_leaves(args)}
        values = {value for _, value in result_leaves(result)} if result is not None else set()
