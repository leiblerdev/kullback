...
from kullback.runner.records import ClassifiedBy, ColumnClass, content_hash, read_jsonl
...
def queue_regrade(workdir: Union[str, Path], key: str, reason: str) -> list[str]:
    ...
    run_ids = sorted({row["run_id"] for row in read_jsonl(Path(workdir) / USES_FILE) if row.get("key") == key})
...
def queued_regrades(workdir: Union[str, Path]) -> list[str]:
    ...
    out: list[str] = []
    for row in read_jsonl(Path(workdir) / QUEUE_FILE):
...
def clear_regrade_queue(workdir: Union[str, Path], run_ids: Optional[Iterable[str]] = None) -> None:
    ...
    kept = [row for row in read_jsonl(path) if row.get("run_id") not in done]
    if not kept:
        path.unlink()
        return
