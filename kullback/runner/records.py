...
class ToolCall(Record):
    ...
    # None means no one recorded either way, which is what lets gates/artifacts.py tell a call
    # whose tool message was never captured apart from a tool that answered with a JSON null (D66).
...
# Usage (tokens on one model call) is defined in ai/usage.py and imported at the top of this
# module: it is what a model call returns, so the provider layer owns it, and ai imports nothing
# of ours (D121). It carries Record's config by hand and is read off Cost below as before.
...
class Run(Record):
    ...
    payload, which is where `verdict.py` and `gates/verifier_suite.py` read them from, so the JSONL
...
_RUN_EVENT_TYPES = frozenset(get_args(EventType))


def read_jsonl(path: Any) -> list[dict]:
    """Every non-blank line of a JSONL file as a dict; a file that is not there reads as no rows.

    canon.py's equivalence files and judge.py's disagreement queue are both read through this, so
    the two side ledgers cannot drift apart in how they treat a blank line or a missing file.
    """
    file = Path(path)
    if not file.is_file():
        return []
...
def plain(value: Any) -> Any:
    ...
    state.py's StateView and route.py's Router use this for the world they read and write; compile_env.py
...
# round-trip test and the pipeline cache treat it like any other.
ALL_RECORDS: tuple[type[BaseModel], ...] = tuple(
