...
from kullback.runner.canon import clear_regrade_queue, queued_regrades
from kullback.runner.records import Verdict, Verifier, as_dict, content_hash
...
def regrade_run(run_jsonl: Any, verifier: Verifier, canon: Any = None, *, out_dir: Any = None,
    ...
    key = cache_key(run.run_id, verifier, environment, runner_version, judge_version, verdict_version,
                    canon=canon, judge_results=judge_results, **verdict_kwargs)
    path = verdict_path(out_dir, run.run_id, key) if out_dir is not None else None
    ...
    result = verdict(
        run, verifier, canon, judge_results,
...
def regrade(run_jsonls: Iterable[Any], verifier: Verifier, canon: Any = None, *, out_dir: Any = None,
    ...
    for run_jsonl in run_jsonls:
        # Parsed once here and handed down as a Run, rather than re-read by regrade_run and again
        # by verdict(); load_run returns a Run unchanged, so every public signature still takes a
        # path, a dict or a Run.
        run = load_run(run_jsonl)
        run_id = run.run_id
        per_run = (judge_results or {}).get(run_id)
        out.append(regrade_run(
            run, verifier, canon, out_dir=out_dir, judge_results=per_run,
