...
from kullback.episode.loading import EnvironmentError, _record, _shaped_json
...
class BuiltEnvironment:
    ...
    def overlay(self, task_id: str) -> tuple[Any, dict]:
        """The Task's overlay and its row values (D74)."""
        return loading.load_overlay(self.root, task_id)
    ...
    def agent_driven(self, task_id: str) -> bool:
        ...
        try:
            ...
            if exc.name not in ("kullback", "kullback.user", "kullback.user.fidelity"):
                raise
            return False
        scores = user_fidelity.load_scores(self.root)
        rows = scores.get("tasks") or ()
