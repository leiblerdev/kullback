...
class SessionStore:
    ...
    def active_path(self) -> list[SessionEntry]:
        ...
        replaces is `replaces_entry_ids` plus, when `first_kept_entry_id` names an entry on the
        path, everything before that entry: tau writes the second shape and older sessions the
        first, and both replay the same. The root `session_info` is never replaced, and a
        compaction that replaces nothing on the path stays where it was appended.
        """
